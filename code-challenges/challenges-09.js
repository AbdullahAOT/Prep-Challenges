...
const roundDecimals = (arr) => {
    let arrayResult=[];
    arr.forEach((element)=>{
        arrayResult.push(Math.round(element));
    });
    return arrayResult;
}
// -------------------------------------------------------------------------------------------------------
...
const employeesBonus = (arr) => {
    let resultArray = [];
    arr.forEach((element) => {
        let transferObject = element;
        let regexForSalary = /\d+/g;
        let salaryDigits = transferObject.salary.match(regexForSalary);
        transferObject.salary = parseInt(salaryDigits.join(""));
        if (transferObject.workHours > 8) {
            transferObject.salary += 100;
        } else {
            transferObject.salary += 50;
        }
        transferObject.salary += "$";
        resultArray.push(transferObject);
    });
    return resultArray;
}
